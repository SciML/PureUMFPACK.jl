...
"""
    splu(A; method=:gplu, ordering=:amd, tol=0.1, scale=SCALE_SUM, delayed=false, check=true) -> PureLU
...
- `delayed` (`:multifrontal` only): enable cross-front (delayed) threshold
  pivoting for general-unsymmetric robustness. Default `false` keeps the
  in-block path bit-identical to previous releases (see [`multifrontal_lu`](@ref)).
"""
function splu(A::SparseMatrixCSC{Tv, Ti}; method::Symbol = :gplu, ordering::Symbol = :amd,
        tol::Real = 0.1, scale::ScaleKind = SCALE_SUM, delayed::Bool = false,
    ...
    if method === :multifrontal
        F = multifrontal_lu(As; tol = tol, delayed = delayed, check = check)
